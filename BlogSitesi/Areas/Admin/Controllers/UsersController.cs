...
namespace BlogSitesi.Areas.Admin.Controllers
{
    ...
    public class UsersController : Controller
    {
        ...
        {
            var user = context.Users.Find(id); // Edit sayfası bizden model olarak içi dolu bir kullanıcı bekliyor. Bunu veritabanından id'ye göre çektik.
            return View(user);
        }
        ...
        public ActionResult Delete(int id, User user)
        {
            try
            {
                context.Users.Remove(user); // Silineceğini söyler
                context.SaveChanges();// ve bu kodla siler
                return RedirectToAction(nameof(Index));
            }
            catch
            {
